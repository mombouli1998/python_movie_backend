...
##dependance pour la session de la bd
def get_db():
    db=SessionLocal()
    try:
        yield db
...
@app.get(
    "/",
    summary="verification de la sangté de l'application",
    description="vérrifie que l'api fonctionne bien",
    response_description="statut de l'api",
    operation_id="health_check_movies",
    tags=["monitoring"]
)
async def root():
    return {"message": "Api Movie opérationnelle"}

####Endpoint pour la récuperation d'un film par son identification
@app.get(
    "/movies/{movie_id}", #/movies/1
    summary="obtenir un film par son id",
    description="Retorune les informations d'un film par son movie_Id.",
    response_description="Details du films",
    response_model=schemas.MovieDetailed,
    tags=["films"]
)
def read_movie(movie_id:int =Path(...,description="l'identifiant unique du fulm"),db:Session= Depends(get_db)):
...
@app.get(
    ...
    description="Retourne une liste de films avec pagination et filtres par titre ou genre",
...
def list_movies(
    ...
    genre:str=Query(None,descriptio="Filtere par genre"),
    db:Session=Depends(get_db)
):
    movies=helpers.get_movies(db,skip,limit,title,genre)
    return movies
##Endpoit pour obtenir une évaluation par utilisateur et film
@app.get(
...
def read_ratind(
    ...
    if rating is None:
        raise HTTPException(
            status_code=404,
            detail=f"Aucune évaluation trouvé par l'utilisateur {user_id} et le film {movie_id}"
        )
    return rating
...
@app.get(
    ...
    description="Retourne une liste d'évaluation avec pagination et filtres optionnels (film, utilisateur, note min).",
...
def list_ratings(
    skip:int=Query(0,ge=0,description="Nombre de résultats à ignorer"),
    limit:int=Query(100,le=1000, description="Nombre maximal de résultats à retourner"),
    movie_id:Optional[int]=Query(None,description="Filtrer par id d'utilisateur"),
    user_id:Optional[int]=Query(None,description="Filrer par ID utilisateur"),
    min_rating:Optional[float]=Query(None,ge=0.0,le=5.0,description="Filtrer les notes supérieures ou égales à cette valeur"),
    db:Session=Depends(get_db)
):
    ratings=helpers.get_ratings(db,skip,limit,movie_id,user_id,min_rating)
    return ratings
